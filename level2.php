<?php 
session_start(); /*start session */

?>

<?php include 'common/common-meta-header.php'; ?>

<!--****Core - Game Engine****-->
<div class="games level1">
    <img class="level2_image" src="images/game3.png" alt="level2_image">
    <table> 
        <!--*****ROW 1*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 2*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 3*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 4*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 5*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 6*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=10 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 7*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=10 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=50 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 8*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=50 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=30 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=30 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 9*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=30 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=20 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 10*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=20 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 11*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=20 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=40 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 12*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=20 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=50 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=40 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 13*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=50 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=40 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 14*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 15*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=50 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 16*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=10 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=50 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 17*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=40 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=10 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=10 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>

        <!--*****ROW 18*****-->
        <tr>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=40 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=40 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=10 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
            <td>
                <form class="input" action="" method="POST" name="level_form">
                    <input type="checkbox" name="score" value=0 checked="checked" hidden="true">
                    <input name="submit" type="submit" class="submit-btn" value="Click">
                </form>
            </td>
        </tr>
    </table>
</div>
<!--****Core - Game Engine End****-->

<!--Information div-->
<div>
    <!--Score goes here-->
    <br/><br/><br/><br/>
    <?php 
    $score = $total = 0;
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST["score"])) {
            $score = (int) $_POST["score"];
        }

        if (isset($_POST["difficulty"])) {
            $_SESSION["difficulty"] = (int) $_POST["difficulty"];
        }
    }

    //Add total score so far
    $total = $_SESSION["score"];
    $total += $score;
    $_SESSION["score"] = $total;//set new score

    //Execute according to difficulty level
    if($_SESSION["difficulty"] == 50 && $total >= 100){
        echo "Congratulation!<br/>";
        echo "You have completed this level with Easy Difficulty<br/>";
        echo "Loading Next Screen...<br/>";

        ?> 
        <form class="input" action="level2.php" method="POST" name="loading">
            <table>
                <tr>
                    <td colspan="2" class="centered">
                        <input name="Submit" type="submit" class="submit-btn" value="Next Level">
                    </td>
                </tr>
            </table>
        </form>
        <?php

    } else if($_SESSION["difficulty"] == 100 && $total >= 200){
        echo "Congratulation!<br/>";
        echo "You have completed this level with Normal Difficulty<br/>";
        echo "Loading Next Screen...<br/>";

        ?> 
        <form class="input" action="level2.php" method="POST" name="loading">
            <table>
                <tr>
                    <td colspan="2" class="centered">
                        <input name="Submit" type="submit" class="submit-btn" value="Next Level">
                    </td>
                </tr>
            </table>
        </form>
        <?php

    } else if($_SESSION["difficulty"] == 200 && $total >= 400){
        echo "Congratulation!<br/>";
        echo "You have completed this level with Hard Difficulty<br/>";
        echo "Loading Next Screen...<br/>";

        ?> 
        <form class="input" action="level2.php" method="POST" name="loading">
            <table>
                <tr>
                    <td colspan="2" class="centered">
                        <input name="Submit" type="submit" class="submit-btn" value="Next Level">
                    </td>
                </tr>
            </table>
        </form>
        <?php

    }
    ?>
    <p>Your score: <?php echo $total; ?></p>

    <!--Logout button-->
    <br/><br/>
    <form class="input" action="logout.php" method="POST" name="login_form">
        <table>
            <tr>
                <td colspan="2" class="centered">
                    <input name="Submit" type="submit" class="submit-btn" value="Logout">
                </td>
            </tr>
        </table>
    </form>
</div>

<?php include 'common/common-footer.php'; ?>